...
ST_FUNC size_t section_add(Section *sec, addr_t size, int align)
{
    ...
    trace ("section_add 08 data_offset="); eputs (itoa (sec->data_offset)); eputs ("\n");
...
ST_FUNC void section_reserve(Section *sec, unsigned long size)
{
    ...
    trace ("section_reserve 08 data_offset="); eputs (itoa (sec->data_offset)); eputs ("\n");
}
...
static void rebuild_hash(Section *s, unsigned int nb_buckets)
{
    ...
    trace_enter ("rebuild_hash");
    strtab = s->link->data;
    nb_syms = s->data_offset / sizeof(ElfW(Sym));

    trace ("rebuild_hash nb_syms="); eputs (itoa (nb_syms)); eputs ("\n");
    ...
    trace ("rebuild_hash 15 nb_syms="); eputs (itoa (nb_syms)); eputs ("\n");
    trace ("rebuild_hash 15 nb_buckets="); eputs (itoa (nb_buckets)); eputs ("\n");

    sym = (ElfW(Sym) *)s->data + 1;
    for(sym_index = 1; sym_index < nb_syms; sym_index++) {
        trace ("rebuild_hash 20");
        if (ELFW(ST_BIND)(sym->st_info) != STB_LOCAL) {
            ...
            *ptr = hash[h];
            ...
            trace ("rebuild_hash 25 h\n");
            *ptr = 0;
        }
        ptr++;
        sym++;
    }
    trace_exit ("rebuild_hash");
}
...
ST_FUNC int find_elf_sym(Section *s, const char *name)
{
    ...
    while (sym_index != 0) {
        ...
        //if (name1 && *name1) {
        //  trace ("find_elf_sym 14 name1= "); eputs (name1); eputs ("\n");
        //}
...
#define for_each_elem(sec, startoff, elem, type) \
  __i = sec->data;\
  eputs ("sec->data="); eputs (itoa (sec->data)); eputs ("\n");\
  eputs ("sizeof type="); eputs (itoa (sizeof (type))); eputs ("\n");\
  __j = sizeof (type) * startoff; \
  eputs ("__j="); eputs (itoa (__j)); eputs ("\n");\
  __i += __j;\
  elem = __i;\
...
ST_FUNC void squeeze_multi_relocs(Section *s, size_t oldrelocoffset)
{
	...
	*dest = *r;
    ...
    trace ("squeeze_multi_relocs data_offset="); eputs (itoa (sr->data_offset)); eputs ("\n");
}
...
ST_FUNC void relocate_syms(TCCState *s1, Section *symtab, int do_resolve)
{
    ...
    for_each_elem(symtab, 1, sym, ElfW(Sym)) {
        ...
        if (sh_num == SHN_UNDEF) {
            ...
            trace ("relocate_syms 22 st_value="); eputs (itoa (sym->st_value)); eputs ("\n");
        }
    found: ;
    }
...
ST_FUNC void relocate_section(TCCState *s1, Section *s)
{
  trace_enter ("relocate_section");
    ...
    trace ("relocate_section 10\n");
    relocate_init(sr);

    rel = sr->data;
    trace ("relocate_section 11 sr->rel->r_offset="); eputs (itoa (rel->r_offset)); eputs ("\n");

    for_each_elem(sr, 0, rel, ElfW_Rel) {
        trace ("relocate_section 12 rel->r_offset="); eputs (itoa (rel->r_offset)); eputs ("\n");
#if 0
        ...
        ptr = s->data + o;
#endif
        trace ("relocate_section 13\n");
        sym_index = ELFW(R_SYM)(rel->r_info);
        trace ("relocate_section 14\n");
        sym = &((ElfW(Sym) *)symtab_section->data)[sym_index];
        trace ("relocate_section 15\n");
        type = ELFW(R_TYPE)(rel->r_info);
        trace ("relocate_section 16\n");
        tgt = sym->st_value;
        trace ("relocate_section 17\n");
#if SHT_RELX == SHT_RELA
        tgt += rel->r_addend;
#endif
        trace ("relocate_section 18\n");
        addr = s->sh_addr + rel->r_offset;
        trace ("relocate_section 19\n");
        relocate(s1, rel, type, ptr, addr, tgt);
        trace ("relocate_section 20\n");
    }
    /* if the relocation is allocated, we change its symbol table */
    trace ("relocate_section 30\n");
    if (sr->sh_flags & SHF_ALLOC)
        sr->link = s1->dynsym;
  trace_exit ("relocate_section");
}
...
static int prepare_dynamic_rel(TCCState *s1, Section *sr)
{
    ...
    if (count) {
        ...
        trace ("prepare_dynamic_rel sh_size="); eputs (itoa (sr->sh_size)); eputs ("\n");
    }
    return count;
}
...
static void alloc_sec_names(TCCState *s1, int file_type, Section *strsec)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        if (file_type == TCC_OUTPUT_DLL &&
            ...
            trace ("alloc_sec_names sh_size="); eputs (itoa (s->sh_size)); eputs ("\n");
    ...
    trace ("alloc_sec_names strsecsh_size="); eputs (itoa (strsec->sh_size)); eputs ("\n");
                           ...
                           struct dyn_inf *dyninf, int *sec_order)
{
    ...
    trace ("layout_sections 5file_offset="); eputs (itoa (file_offset)); eputs ("\n");
    ...
    if (phnum > 0) {
        ...
        if (s1->has_text_addr) {
            ...
            trace ("layout_sections 15file_offset="); eputs (itoa (file_offset)); eputs ("\n");
        } else {
        ...
        for(j = 0; j < 2; j++) {
            ...
            trace ("layout_sections 40file_offset="); eputs (itoa (file_offset)); eputs ("\n");
            ph->p_type = PT_LOAD;
            if (j == 0)
            ...
            for(k = 0; k < 5; k++) {
                ...
                trace ("layout_sections 50file_offset="); eputs (itoa (file_offset)); eputs ("\n");
                for(i = 1; i < s1->nb_sections; i++) {
                    s = s1->sections[i];
                    ...
                    trace ("layout_sections 60file_offset="); eputs (itoa (file_offset)); eputs ("\n");
                    ...
                    trace ("layout_sections 80file_offset="); eputs (itoa (file_offset)); eputs ("\n");
                }
                trace ("layout_sections 81file_offset="); eputs (itoa (file_offset)); eputs ("\n");
            }
            trace ("layout_sections 82file_offset="); eputs (itoa (file_offset)); eputs ("\n");
	    if (j == 0) {
		...
		ph->p_paddr &= ~(ph->p_align - 1);
            ...
            if (j == 0) {
                if (s1->output_format == TCC_OUTPUT_FORMAT_ELF) {
                    ...
                    trace ("layout_sections 90file_offset="); eputs (itoa (file_offset)); eputs ("\n");
                }
            }
        }
    }
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        file_offset = (file_offset + s->sh_addralign - 1) &
            ~(s->sh_addralign - 1);
#else
        trace ("layout_sections 83file_offset="); eputs (itoa (file_offset)); eputs ("\n");
        file_offset = (file_offset + s->sh_addralign - 1);
        trace ("layout_sections 84file_offset="); eputs (itoa (file_offset)); eputs ("\n");
        int align = (s->sh_addralign - 1);
        trace ("layout_sections 85align="); eputs (itoa (align)); eputs ("\n");
        align = ~align;
        trace ("layout_sections 86align="); eputs (itoa (align)); eputs ("\n");
        file_offset = file_offset & align;
        trace ("layout_sections 87file_offset="); eputs (itoa (file_offset)); eputs ("\n");
#endif
        trace ("layout_sections 88file_offset="); eputs (itoa (file_offset)); eputs ("\n");
        s->sh_offset = file_offset;
        trace ("layout_sections 89 sh->sh_type="); eputs (itoa (s->sh_type)); eputs ("\n");
        if (s->sh_type != SHT_NOBITS) {
            trace ("layout_sections 90 sh->sh_size="); eputs (itoa (s->sh_size)); eputs ("\n");
            file_offset += s->sh_size;
            trace ("layout_sections 91file_offset="); eputs (itoa (file_offset)); eputs ("\n");
        }
...
static void fill_dynamic(TCCState *s1, struct dyn_inf *dyninf)
{
    ...
    trace ("fill_dynamic data_offset="); eputs (itoa (dynamic->data_offset)); eputs ("\n");
                           ...
                           int file_offset, int *sec_order)
{
    ...
    for(i = 0; i < s1->nb_sections; i++) {
        ...
        if (s) {
            ...
            trace ("tcc_output_elf s->sh_size="); eputs (itoa (s->sh_size)); eputs ("\n");
        }
...
{
    trace_enter ("tcc_object_type");
    trace ("tcc_object_type sizeof="); eputs (itoa (sizeof(ElfW(Ehdr)))); eputs ("\n");
    int SIZEOF = 52;
    // int size = read(fd, h, sizeof(ElfW(Ehdr)));
    int size = read(fd, h, sizeof *h);
    trace ("tcc_object_type size="); eputs (itoa (size)); eputs ("\n");
    // if (size == sizeof(ElfW(Ehdr)) && 0 == memcmp(h, ELFMAG, 4)) {
    if (size == sizeof *h && 0 == memcmp(h, ELFMAG, 4)) {
        trace ("tcc_object_type h->etype="); eputs (itoa (h->e_type)); eputs ("\n");
        if (h->e_type == ET_REL)
          {
            trace_exit ("tcc_object_type@0");
            return AFF_BINTYPE_REL;
          }
        if (h->e_type == ET_DYN)
          {
            trace_exit ("tcc_object_type@1");
            return AFF_BINTYPE_DYN;
          }
    } else if (size >= 8) {
        if (0 == memcmp(h, ARMAG, 8))
          {
            trace_exit ("tcc_object_type@2");
                                ...
                                int fd, unsigned long file_offset)
{
    ...
    found:
        ...
        trace ("tcc_load_object_file data_offset="); eputs (itoa (s->data_offset)); eputs ("\n");
    no_align:
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type == SHT_RELX) {
            ...
            trace ("tcc_load_object_file 129\n");
        }
    }
    sm = sm_table;
    ...
    for(i = 1; i < nb_syms; i++, sym++) {
        trace ("tcc_load_object_file 140\n");
        trace ("tcc_load_object_file 140 SH_UNDEF="); eputs (itoa (SHN_UNDEF));  eputs ("\n");
        trace ("tcc_load_object_file 140 SH_LORESERVE="); eputs (itoa (SHN_LORESERVE));  eputs ("\n");
        trace ("tcc_load_object_file 140 sym->st_shndx="); eputs (itoa (sym->st_shndx)); eputs ("\n");
        trace ("tcc_load_object_file 140 sym->st_shndx="); eputs (itoa (sym->st_shndx)); eputs ("\n");
        if (sym->st_shndx != SHN_UNDEF &&
            sym->st_shndx < SHN_LORESERVE) {
            trace ("tcc_load_object_file 141\n");
        ...
        /* add symbol */
        name = (char *) strtab + sym->st_name;
        trace ("tcc_load_object_file 150 name="); eputs (name); eputs ("\n");
...
ST_FUNC int tcc_load_archive(TCCState *s1, int fd)
{
    ...
    for(;;) {
        ...
        if (!strcmp(ar_name, "/")) {
            ...
            if(s1->alacarte_link)
              {
                trace_exit ("tcc_load_archive 30");
                return tcc_load_alacarte(s1, fd, size, 4);
              }
	} else if (!strcmp(ar_name, "/SYM64/")) {
            if(s1->alacarte_link)
              {
                trace_exit ("tcc_load_archive 40");
            ...
            if (tcc_object_type(fd, &ehdr) == AFF_BINTYPE_REL) {
                if (tcc_load_object_file(s1, fd, file_offset) < 0)
                  {
                    trace_exit ("tcc_load_archive 50");
                    return -1;
                  }
            }
        }
